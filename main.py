...
class HTTPResponse(HTTP):
	...
	def setCode(self, code):
		...
		self.data = ('<html><body><h1>%s %s</h1><p>%s</p></body></html>' % (self.code, self.status, info)).encode('utf-8')
	...
	def 	make_response(self, http_request):
		...
		if is_file:
			...
			if last_time and time.mktime(time.strptime(last_time, "%a, %d %b %Y %H:%M:%S GMT"))>os.stat(path).st_mtime:
				self.setCode('304')
			else:
				self.setCode('200')
